...
export default function ExecTab() {
      ...
      datasets: [
        {
          ...
          backgroundColor: 'rgba(232,87,42,0.7)',
          borderRadius: 4,
          yAxisID: 'y',
        },
        {
  ...
  const actionItems = [
    { priority: '🔴', text: 'AWTA 2차 취합 마감 임박 (4/11)', detail: '법인별 과업정의서 제출 모니터링 필요' },
    { priority: '🟠', text: 'OK저축은행 이직률 상승 추세', detail: '전년 대비 2.1%p 상승, 핵심인력 리텐션 전략 검토' },
    { priority: '🟡', text: '1인당 인건비 3년 연속 증가', detail: '생산성 지표 대비 인건비 효율성 모니터링' },
  ];

  return (
    <div>
      <div className="kpi-row">
        <KpiCard label="그룹 인원" value={fd.totalHeadcount.toLocaleString() + '명'} sub={`YoY ${fd.yoy > 0 ? '+' : ''}${fd.yoy}%`} trend={fd.yoy >= 0 ? 'up' : 'down'} />
        <KpiCard label="이직률" value={fd.turnoverRate + '%'} />
        <KpiCard label="인건비" value={fd.totalLaborCost.toLocaleString() + 'M'} />
        <KpiCard label="1인당 인건비" value={fd.perCapita.toLocaleString() + 'M'} />
      </div>
